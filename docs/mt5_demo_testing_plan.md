...
| VWAP computed from a truncated 100-bar window, not true session VWAP | M5 confirmation's `close > vwap` check may not reflect a genuine session VWAP (documented ~2.4% divergence found during review) | Occasionally cross-check `m5_confirmation` decisions against a real chart's VWAP by eye |
